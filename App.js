...
const customFonts = {
  QuicksandBold: require("./assets/Quicksand-Bold.ttf"),
  QuicksandRegular: require("./assets/Quicksand-Regular.ttf"),
  QuicksandSemiBold: require("./assets/Quicksand-SemiBold.ttf"),
};
...
const AppScreen = () => {
  ...
  if(isLoading) {
    ...
    console.log('font:',isFontLoaded, isLoading)
    console.log('LOADED!')
...
const HomeSearch = () => {
  ...
  return(
    <View style={{flex: 69, justifyContent:'flex-end', alignItems: 'center', flexDirection: 'row', marginTop: 5}}>
...
const HomeYesterdayWeather = ({data, maxMinTemp}) => {
  ...
  return(
    <View style={{flex: 276, justifyContent: 'center', alignItems: 'center', paddingVertical: 10}}>
      <Text style={[styles.centertext,{fontSize: 16, fontFamily: 'QuicksandBold'}]}>{data.location.name}, {data.location.region}</Text>

      <Text style={[styles.centertext,{fontFamily: 'QuicksandRegular'}]}>{data.location.localtime}</Text>

      <View style={{flex:2, flexDirection: 'row', alignItems: 'center'}}>
        <Image tintColor={'white'} source={getWeatherImgUri(data.forecast.forecastday[0].day.condition.icon)} style={{width: 64, height: 64}}></Image>
        <Text style={{textAlignVertical: 'center', color: 'white', fontSize: 60, fontFamily: 'QuicksandSemiBold'}}>{Math.round(data.current.temp_c)}°</Text>
      </View>
      

      <Text style={[styles.centertext,{fontFamily: 'QuicksandBold'}]}>{maxMinTemp[0]}°/{maxMinTemp[1]}° Feels like {Math.round(data.current.feelslike_c)}°</Text>
      
      <Text style={[styles.centertext,{fontFamily: 'QuicksandBold', paddingBottom: 20}]}>{data.forecast.forecastday[0].day.condition.text}</Text>
    </View>
  )
}
...
const getUVlevel = () => {
  ...
  return(
    <View style={{flex: 135}}>
      <Text style={{textAlign:'right', paddingHorizontal: 30, color: 'white',paddingBottom:3, fontFamily: 'QuicksandSemiBold'}}>Yesterday: {maxMinTemp[0]}°/{maxMinTemp[1]}°</Text>
      <View style={styles.bordercontainer}>
        <View style={{flex: 1, margin: 5, alignItems: 'center', flexDirection: 'row', justifyContent: 'center'}}>
          ...
          <Text style={{textAlignVertical: 'center', color: 'white', fontFamily: 'QuicksandSemiBold'}}>Precipitation{"\n"}{data.current.humidity}%</Text>
        </View>
        ...
        <View style={{flex: 1, margin: 5, alignItems: 'center', flexDirection: 'row', justifyContent: 'center'}}>
          ...
          <Text style={{textAlignVertical: 'center', color: 'white', fontFamily: 'QuicksandSemiBold'}}>UV Index{"\n"}{getUVlevel()}</Text>
        </View>
      </View>
    </View>
  )
...
const HomeFullDayWeather = ({data, maxMinTemp}) => {
  ...
  return (
    <View style={{flex: 260}}>
      <Text style={{textAlign:'left', paddingHorizontal: 30, color: 'white',paddingBottom:3, fontFamily: 'QuicksandSemiBold'}}>Hourly</Text>
      <View style={styles.bordercontainer}>
...
const WeatherColum = ({ dayData, maxMinTemp}) => {
  ...
  return(
    <View style={{flex: 1, margin: 5, alignItems: 'center'}}>
      <Text style={[styles.centertext, {flex: 0.5, textAlignVertical: 'bottom', marginTop: 10, fontFamily: 'QuicksandBold'}]}>{dayData.time.slice(-5)}</Text>

      <View style={styles.centertext}>
        <Image tintColor={'white'} source={getWeatherImgUri(dayData.condition.icon)}  resizeMode={'contain'}  style={{flex: 2, width: 64, height: 64, alignSelf: 'center'}}></Image>
      ...
      <View style={{
        ...
        <Text style={{alignSelf: 'flex-end', color: 'white', fontSize: 22, fontFamily: 'QuicksandSemiBold'}}> {Math.round(dayData.temp_c)}°</Text>
